/** Exact coordinated v2 claims. No local clock or transport iat may replace auth_time. */
export type IdentityAuthentication = { method: 'webauthn_uv'; auth_time: number }
...
export function snapshotIdentityAuthentication(value: unknown): IdentityAuthentication | null {
  try {
    ...
    if (!method?.enumerable || !time?.enumerable || !('value' in method) || !('value' in time)
      || method.value !== 'webauthn_uv' || typeof time.value !== 'number'
      || !Number.isSafeInteger(time.value) || time.value < 0) return null
    return Object.freeze({ method: 'webauthn_uv', auth_time: time.value })
  } catch { return null }
}

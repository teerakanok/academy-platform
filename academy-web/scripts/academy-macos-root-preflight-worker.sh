...
if [[ -e "$stage" ]]; then
  [[ "$(stat -f '%Su:%Sg:%Lp' "$stage")" == 'root:wheel:700' ]]
  [[ -f "$stage/.academy-owned" && ! -L "$stage/.academy-owned" ]]
  [[ "$(stat -f '%Su:%Sg:%Lp:%l' "$stage/.academy-owned")" == 'root:wheel:400:1' ]]
  [[ "$(cat "$stage/.academy-owned")" == 'academy-root-preflight/d6e517e3' ]]
  /bin/rm -rf "$stage"
fi
install -d -o root -g wheel -m 700 "$stage" "$stage/source" "$stage/tooling"
printf 'academy-root-preflight/d6e517e3\n' > "$stage/.academy-owned"
chmod 400 "$stage/.academy-owned"
...
for name in academy-release-cli.mjs academy-release-install.mjs academy-release-manifest.mjs academy-release-pointer.mjs academy-release-render.mjs; do
  cp "$repo/scripts/$name" "$stage/tooling/$name"
done
jq -c --arg old "$source" --arg new "$stage/source" \
  'walk(if type == "string" then gsub($old; $new) else . end)' "$input" > "$stage/package.json"
chown -R root:wheel "$stage"
find "$stage/source" "$stage/tooling" -type d -exec chmod 500 {} +
find "$stage/source" "$stage/tooling" -type f -exec chmod 400 {} +
chmod 500 "$stage/source/node" "$stage/tooling/academy-release-cli.mjs"
chmod 600 "$stage/package.json"
verify_root_file() {
  local path=$1 mode=$2 expected=$3
  [[ -f "$path" && ! -L "$path" && "$(stat -f '%Su:%Sg:%Lp:%l' "$path")" == "root:wheel:$mode:1" ]]
...
umask 077
jq -e --arg revision "$revision" --arg sha "$release_sha" \
  '.status == "RENDERED" and .releaseRevision == $revision and .releaseSha256 == $sha' "$stage/render-result.json" >/dev/null
...
[[ "$(shasum -a 256 /opt/academy/production-operations/academy-poola-production-producer.mjs | awk '{print $1}')" == 4a1db4922fdb39b5b8841adb186b39e463a9e53448abb8d19bb4ea21198afc15 ]]
ensure_root_directory() {
  local path=$1
  if [[ -e "$path" ]]; then
    [[ -d "$path" && ! -L "$path" && "$(stat -f '%Su:%Sg:%Lp' "$path")" == 'root:wheel:700' ]]
  else
    install -d -o root -g wheel -m 700 "$path"
  fi
}
...
install_or_match() {
  ...
  if [[ -e "$target" ]]; then
    [[ -f "$target" && ! -L "$target" && "$(stat -f '%Su:%Sg:%Lp:%l' "$target")" == "root:wheel:$mode:1" ]]
    [[ "$(shasum -a 256 "$target" | awk '{print $1}')" == "$expected" ]]
  else
    install -o root -g wheel -m "$mode" "$source_file" "$target"
    created=true
    if [[ ! -f "$target" || -L "$target" || "$(stat -f '%Su:%Sg:%Lp:%l' "$target")" != "root:wheel:$mode:1" \
      || "$(shasum -a 256 "$target" | awk '{print $1}')" != "$expected" ]]; then
...
[[ "$(shasum -a 256 "$db_root/database-config.json" | awk '{print $1}')" == 4cb0613a51b783d48d596551b25f6c8044ea13325cc848f47d57601f035c4c21 ]]
[[ "$(shasum -a 256 /opt/academy/production-operations/p1-p7-config.json | awk '{print $1}')" == 0be8bd9b822047919f06dbf1662bc6e24d0b1a393945fe70220603069ad74e0b ]]

release=/opt/academy/releases/$release_sha
whoami=/private/var/root/academy-wrangler-whoami-d6e517e3.txt
valid_whoami() {
  [[ -f "$whoami" && ! -L "$whoami" && "$(stat -f '%Su:%Sg:%Lp:%l' "$whoami")" == 'root:wheel:600:1' ]]
  local size="$(stat -f '%z' "$whoami")"
...
if [[ -e "$whoami" || -L "$whoami" ]]; then
  ...
  local_size="$(stat -f '%z' "$whoami_tmp")"
  [[ "$local_size" -gt 0 && "$local_size" -le 65536 ]]
  chmod 600 "$whoami_tmp"
  /bin/ln "$whoami_tmp" "$whoami"
  /bin/rm "$whoami_tmp"
  trap - EXIT
  valid_whoami
fi
chmod 600 "$stage"/*-result.json

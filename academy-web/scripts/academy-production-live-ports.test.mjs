...
async function fixture(t) {
  ...
  for(const name of ['inspectRecovery','backupRestore','applyMigrations','uploadCandidate','activateTraffic','smokeP1P7','rollbackTraffic','checkResidue']){
    ...
    operations[name]={executable,sha256:createHash('sha256').update(body).digest('hex'),args:['--authority','{AUTHORITY_ID}','--release','{RELEASE_REVISION}']}
  }
  ...
  operations.activateTraffic.args=['--expected-deployment','{EXPECTED_DEPLOYMENT_ID}','--expected-version','{EXPECTED_VERSION_ID}','--candidate','{CANDIDATE_VERSION_ID}']
  ...
  return{authorityPath,operations}
}
...
test('adapter binds protected authority, pinned executables and Cloudflare CAS',async t=>{const f=await fixture(t),calls=[];const run=async call=>{calls.push(call);return{status:0,stdout:JSON.stringify(call.operation==='inspectRecovery'?deployment():{status:'PASS',receiptSha256:D})}};const ports=await createAcademyProductionLivePorts({authorityPath:f.authorityPath,run,clock:()=>NOW});assert.deepEqual(await ports.discoverCurrent(),{deploymentId:ids.deployment,versionId:ids.version});await ports.activateTraffic({expectedCurrentDeploymentId:ids.deployment,expectedCurrentVersionId:ids.version,candidateVersionId:ids.candidate,traffic:100});assert.ok(calls.at(-1).args.includes(ids.candidate));assert.equal(calls.at(-1).args.some(value=>value.includes('secret-value')),false)})

test('adapter rejects expired authority before command execution',async t=>{const f=await fixture(t);let calls=0;await assert.rejects(createAcademyProductionLivePorts({authorityPath:f.authorityPath,run:async()=>{calls++},clock:()=>Date.parse('2026-08-29T07:00:00Z')}));assert.equal(calls,0)})

test('adapter rejects executable digest drift',async t=>{const f=await fixture(t);await writeFile(f.operations.backupRestore.executable,'#!/bin/sh\nexit 1\n');await assert.rejects(createAcademyProductionLivePorts({authorityPath:f.authorityPath,run:async()=>({status:0,stdout:'{}'}),clock:()=>NOW}))})

test('adapter fails CAS before traffic mutation',async t=>{const f=await fixture(t),calls=[];const run=async call=>{calls.push(call);return{status:0,stdout:JSON.stringify(deployment())}};const ports=await createAcademyProductionLivePorts({authorityPath:f.authorityPath,run,clock:()=>NOW});await assert.rejects(ports.activateTraffic({expectedCurrentDeploymentId:ids.deployment,expectedCurrentVersionId:ids.candidate,candidateVersionId:ids.candidate,traffic:100}));assert.equal(calls.some(x=>x.operation==='activateTraffic'),false)})

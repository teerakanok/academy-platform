...
const AUTHORITY = await verifyIdentityProductionAuthority(new Date('2026-08-29T03:10:00.000Z'))
...
test('CLI restart publishes a retained terminal journal before provider discovery and leaves no journal', async t => {
  ...
  await runAcademyProductionActivation({authority:AUTHORITY,plan,ports,release:ACTIVATION_RELEASE,observedAt:nowDate,journalPath,receiptPath})

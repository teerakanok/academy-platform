...
#
# ข้ออ้างเชิงตัวเลขที่วัดจากเครื่อง (6.91x, 3.85x, 1.93x) ขึ้นกับฮาร์ดแวร์
...
if $CC -O0 -o "$LAB/br0" "$LAB/branch.c" 2>/dev/null; then
  r=$("$LAB/br0")
  # บทอ้างว่าข้อมูลที่ทำนายไม่ได้แพงกว่า "มาก" ที่ -O0 (เครื่องผู้เขียน 6.91x)
  if python3 -c "import sys;sys.exit(0 if $r > 2.0 else 1)"; then
    ok "-O0: ไม่เรียงแพงกว่าเรียงอย่างมีนัย (${r}x, บทอ้าง 6.91x บนเครื่องผู้เขียน)"
  else
...
if $CC -O2 -o "$LAB/dep" "$LAB/dep.c" 2>/dev/null; then
  ...
  if python3 -c "import sys;sys.exit(0 if $r > 1.5 else 1)"; then
    ok "โซ่อิสระเร็วกว่าโซ่เดียวอย่างมีนัย (${r}x, บทอ้าง 3.85x บนเครื่องผู้เขียน)"
  else
...
if $CC -O2 -o "$LAB/lu" "$LAB/lu.c" 2>/dev/null; then
  ...
  if python3 -c "import sys;sys.exit(0 if $1 > 1.2 else 1)"; then
    ok "load ที่พึ่งพากันแพงกว่า load อิสระ ทั้งที่อยู่ใน L1 ทั้งคู่ (${1}x, บทอ้าง 1.93x)"
  else
...
if [ -n "$line" ] && $CC -O2 -o "$LAB/fs" "$LAB/fs.c" -lpthread 2>/dev/null; then
  r=$("$LAB/fs" "$line")
  if python3 -c "import sys;sys.exit(0 if $r > 1.5 else 1)"; then
    ok "ตัวนับ 4 ตัวในบรรทัดเดียวช้ากว่าเติมช่องว่าง ${r}x (บทอ้าง 3.91 เท่า) — ผลลัพธ์ถูกต้องทั้งคู่"
  else

...
**ยังเปิดอยู่ (จดเป็นงานแล้ว ไม่ใช่ปล่อยผ่าน):**
- attempt เก็บแค่ค่าตัวแปร ไม่ได้ snapshot โจทย์จำลองทั้งชิ้น → deploy ระหว่างทางทำให้
  ตรวจด้วยกติกาใหม่ทั้งที่ผู้เรียนอ่านโจทย์เก่า
- `node_progress` ไม่บันทึกว่า attempt/version ไหนทำให้ผ่าน (ใบรับรอง W4 ต้องใช้)
- reload กลาง attempt ทิ้งคำตอบและกินโควตาใหม่ทุกครั้ง
- consume+บันทึกไม่ atomic · ไม่มี retention job

...
| SEC-ACADEMY-016 | Least privilege (DB role) | migration 0030 revokes direct `course_entitlement`/`service_activation` writes, removes dead runtime helpers, and preserves callback/session + lifecycle activation RPCs | DB APPLIED — OPERATOR/APP PROOF OPEN | 2026-09-06 | Actual Pool A ROLLBACK then exact-body COMMIT and read-only privilege checks passed; see reports/operations/20260906-academy-0029-0033. Dedicated operator password remains unset; authenticated operator/application proof remain. ; Worker bc1c738 version43 now active100%, host/Access GET and Chrome captures passed; reports/operations/20260906-academy-bc1c738-release (not authenticated application proof) |
...
| SEC-ACADEMY-019 | Operator script SQL | psql variables/parameterised client; remote-side UUID re-check; injection unit test | LOCAL VERIFIED — PRODUCTION PROOF OPEN | 2026-09-07 | PostgreSQL16 17/17, SSH/runner 8/8 and canonical receipt 21/21; independent review PASS. Host/producer pins and operation identity propagated. Exact operator installation/live canary pending. Transaction1e2b0ff adds actual ROLLBACK-before-COMMIT, lock2s/statement8s and12s processdeadline; root realPG8/8. Four reviewed operator commits now integrated; reports/verification/2026-09-08-operator-integration |
| SEC-ACADEMY-020 | DB privileges (shared role) | migration 0030 revokes Academy staff/control and authorization-table/RPC grants from shared `service_role`, plus `postgres` staff membership; dedicated direct logins are required by both operator scripts | DB APPLIED — OPERATOR/APP PROOF OPEN | 2026-09-06 | Actual Pool A ROLLBACK then exact-body COMMIT and read-only privilege checks passed; see reports/operations/20260906-academy-0029-0033. Dedicated operator password remains unset; authenticated operator/application proof remain. ; Worker bc1c738 version43 now active100%, host/Access GET and Chrome captures passed; reports/operations/20260906-academy-bc1c738-release (not authenticated application proof) |

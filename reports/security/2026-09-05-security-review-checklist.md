...
| SEC-ACADEMY-001 | Rate limiting | เพิ่ม edge rule (method-aware) สำหรับ `/api/auth/identity/start` GET/POST และ `/auth/callback` GET; fail closed เมื่อไม่มี marker; pending-row ceiling ใน `create_identity_authorization_transaction`; Turnstile/WAF ก่อน public | CODE FIXED — INDEPENDENT PASS; PRODUCTION PENDING | 2026-09-06 | Aggregate cap migration0029; real PostgreSQL18/18 and unit2146/2146 passed; independent review PASS; dry-run-first production apply pending; DB0029–0033 applied and independently read back 2026-09-06; Worker upload rejected10021, release correction pending |
...
| SEC-ACADEMY-006 | Assessment integrity | bank ≥3× + sample/shuffle; backoff + daily cap ใน `issue_attempt`; alert; clamp `ATTEMPT_MAX_PER_WINDOW` ใน production; dwell time | LOCAL CONTRACT PASS — CONTENT/PROD OPEN | 2026-09-06 | SQL2/2, unit2417, independent PASS; content banks and production pending; DB0029–0033 applied and independently read back 2026-09-06; Worker upload rejected10021, release correction pending |
| SEC-ACADEMY-007 | Rate-limit path normalisation | canonical trailing slash + exact method:path lookup; one strict decode rejects protected disguises while preserving valid encoded public paths; production fail-closed marker; gated local fixture bypass | LOCAL CODE VERIFIED — PENDING PRODUCTION | 2026-09-06 | RED then GREEN focused 21/21; full unit 2411/2411; URL parser raw-byte normalization limitation documented; parent real workerd13checks and build:cf exit0; independent review PASS; production proof pending |
| SEC-ACADEMY-008 | Rate-limit key granularity | durable IPv6 /64 actor aggregation; bounded HMAC target budgets; global per-route ceilings; malformed IP/target fail-safe handling | LOCAL CODE VERIFIED — PENDING PRODUCTION | 2026-09-06 | Focused RED then GREEN 21/21 covers same/other /64, target reuse, ceilings, partial failure, stream preservation; parent actual workerd enforcement/marker13checks and build:cf exit0; independent review PASS; production proof pending |

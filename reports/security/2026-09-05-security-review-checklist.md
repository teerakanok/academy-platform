...
- **Sources:** both (Fable #12/#16/#36a, GLM medium) · **Status:** CODE CHANGED — 2026-09-06 the existing create RPC now enforces an advisory-lock-protected database-wide outstanding cap; focused unit tests and TypeScript pass, but local disposable-PostgreSQL execution and full build remain environment-blocked. Production/deployment is **NOT GREEN** and still requires migration, independent review, and release authorization.
...
- **Sources:** Fable (#2/#10/#26/#29/#41) · **Status:** CLOSED — ORIGINAL AUTH GATE VERIFIED, 2026-09-08. Independent review PASS; source fe4220a / Worker3830694b@100; owner Safari Dashboard and `/api/auth/me` signedIn=true. Evidence: `reports/verification/2026-09-08-active-user-shape/`.
...
- **Sources:** both (Fable #0/#9/#31, GLM medium) · **Status:** LOCAL VERIFIED — INDEPENDENT REVIEW PASS; PRODUCTION PENDING. Owned PostgreSQL covers revoke isolation, config-revision fencing, callback health gating, observed-successor recovery, and deletion/session serialization. LOW defense-in-depth residual: durable state accepts any structurally canonical issuer instead of pinning the documented exact principal issuer.
...
- **Local correction (2026-09-06):** assessed admission now has a DB-side UTC daily ceiling, consecutive-failure backoff, production quota clamp, and per-task dwell guard; issuance still serializes per learner/node with a transaction advisory lock. The route samples at most five snapshot questions (all three-to-five currently authored), preserves choice shuffling/answer keys, and emits bounded identifier-free integrity warnings. Exact focused gate and both TypeScript gates exit 0; full unit exits 0. A transactional integration fixture exists but was not executable because `TEST_DATABASE_URL`/Docker is unavailable. Production migration/smoke remains pending.
- **Canonical content dependency:** every certificate-bearing capstone still needs at least 15 authored MCQs in Crucible (3× the five-question serve ceiling); current authored capstone banks max out at five, so content-lane expansion is still required for full SEC006 closure.
- **Sources:** Fable (#30/#34) · **Status:** LOCAL CODE VERIFIED — DB FIXTURE, CONTENT, AND PRODUCTION PENDING
...
- **Local correction (2026-09-06):** edge admission canonicalizes one trailing slash before the exact method:path lookup, preserves the query, strictly decodes encoded paths, rejects protected-route disguises and ambiguous slash/backslash/control/dot/double-encoded syntax with `404`, preserves harmless encoded public paths, and removes production in-memory marker fallback. URL parsing can normalize raw invalid bytes before Worker observation; explicit local fixtures retain only flag+loopback-gated bypass. Focused RED then GREEN: 21/21 (`edge-rate-limit` + `identity-security-admission`); full unit 2411/2411; lint reports only baseline 3 `no-require-imports` errors.
- **Sources:** Fable (#19/#36b) · **Status:** LOCAL CODE VERIFIED — PENDING PRODUCTION
...
- **Local correction (2026-09-06):** durable actor objects now aggregate valid IPv6 /64 prefixes (IPv4 remains /128; IPv4-mapped addresses use their embedded IPv4) and reject malformed addresses fail-closed. OTP/verify/leads/unsubscribe add bounded keyed target objects read from a cloned body with a 16,384-byte cap; Identity navigation derives no email target. Every protected request also checks an HMAC-scoped global route ceiling. Unit tests prove same-/64 reuse, other-/64 separation, different IPs/same target, opaque names, route exhaustion, later-scope failure => 503, and preservation of the original request stream. No production deploy or live Cloudflare proof is included.
- **Sources:** Fable (#20) · **Status:** LOCAL CODE VERIFIED — PENDING PRODUCTION
...
- **Evidence:**
  ...
  ```
- **Remediation:** เก็บ `sha256(sessionId)` (base64url 43 ตัว พอดี constraint เดิม) เป็น key และ digest ใน adapter ก่อนทุก RPC (create/read/revoke/claim/finalize); cookie ยังถือค่าดิบ; migration 0034 hash แถวเดิมใน place โดยไม่ expire/delete
- **Local correction (2026-09-07):** both Postgres adapters now keep SHA-256 base64url identifiers on the wire and map them back to the browser bearer only in process memory. Authorization completion derives one stable raw session from the state plus raw browser-binding secret, so response-loss retries and resumed checkpointed completion return the same receipt without a raw-bearer fallback. Migration `0034` hashes existing session/completed-receipt rows, preserves existing cookies, and clears only the old random linkage on unfinished callbacks so the 30-second lease/retry path re-derives a stable receipt. RED focused gate exited `1` (4 failures/86 tests); final GREEN exited `0` (`87/87`, including exact-one cookie parsing). Rollback is intentionally blocked and requires a verified pre-migration restore plus an explicit live-session decision. No production DB was accessed.
- **Sources:** Fable (#13/#22/#37) · **Status:** LOCAL CODE VERIFIED — PENDING MIGRATION REVIEW/PRODUCTION

- **Production closure (2026-09-07; cutover 2026-09-06T22:45:39–22:50:26Z):** exact migration0034 ran under the independently reviewed transaction wrapper, first ROLLBACK with identical schema/owners then COMMIT with aggregate digest/row-preservation checks. Read-only production confirmed marker1, invalid digest0, five digest RPCs and runtime grants; reviewed app0e4417ec now serves Worker1af77a26 at100%. See [command-matched cutover evidence](../operations/20260906-academy-0034-cutover/README.md). Backup is protected and retained; never reapply0034 or restore the old raw-claim app after COMMIT/unknown outcome. This closes the at-rest finding, not the separate authenticated learner journey. No unexpired120-second compatibility representative existed during the production rehearsal; local active-cookie compatibility remains the available proof for that case.
...
- **Local correction (2026-09-07):** grants now sign a SHA-256 base64url digest of the exact opaque Academy session (never the raw bearer); issuance requires the verified session and existing course authorization. Worker delivery parses exactly one well-formed session cookie and compares the digest constant-time before R2; deterministic local delivery enforces the same binding. GET/HEAD/range, 5-minute TTL, path-scoped HttpOnly grant cookie, and no-store remain unchanged. True RED exited 1 (4 failures); GREEN exited 0 (60/60 focused) and full unit exited 0 (2,441 passed/2 skipped). Worker TypeScript exited 0, but local workerd/build gates were blocked by sandbox EPERM on `listen 127.0.0.1`; production and independent review remain pending. See [SEC-ACADEMY-010 evidence](2026-09-07-sec-academy-010-session-bound-media-grants.md).
- **Sources:** Fable (#1/#15/#23) · **Status:** DEPLOYED — PENDING AUTHENTICATED MEDIA ACCEPTANCE
...
- **Sources:** GLM (medium) · **Status:** SOURCE VERIFIED — PRODUCTION OPEN (2026-09-09)
...
- **Sources:** Fable (#5/#6) · **Status:** CANDIDATE — REVIEW/PRODUCTION PROOF OPEN
...
| DIM-04 | Input validation & API abuse | `edge-rate-limit-policy.ts`, DO limiter, `mutation-security.ts`, `bounded-body.ts` ใช้ใน leads/otp/verify/progress/practice; canonical edge admission + actor/target/global budgets; production marker fallback ถูกลบ; `formData()` ใน start | CHECKED | 2026-09-06 | SEC-ACADEMY-001/005/007/008/017; production proof pending |
...
| NC-08 | Lifecycle libraries | Review runtime composition, reducer/page-store/pull transport, config-health gating, observed-successor reconciliation, replay/downgrade fencing, and lock serialization against the frozen local candidate | CHECKED — INDEPENDENT PASS | 2026-09-06 | Production migration/deploy and live producer integration remain unverified; LOW exact durable issuer pinning remains defense in depth |

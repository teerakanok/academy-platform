...
- ความเสี่ยงต่อลูกค้า: วันนี้ canonical host อยู่หลัง Cloudflare Access และ raw `workers.dev` route ถูกปิดใน `c6a67e0` (ยังไม่ deploy) → exposure ต่อสาธารณะต่ำ; แต่ทุกข้อข้างบนกลายเป็น live ทันทีที่เปิด public. ความน่าเชื่อถือของใบรับรอง: SEC-ACADEMY-006 — capstone 4–5 ข้อ ×4 ตัวเลือก, โจทย์ชุดเดิมทุก attempt, oracle ตอบแค่ pass/fail, quota 3/30 นาที → script ผ่าน capstone ได้ใน ~0.9–3.6 วันโดยไม่มี alert.
...
- **Sources:** Fable (#2/#10/#26/#29/#41) · **Status:** OPEN
...
- **Severity:** HIGH (as-was) · **Verdict:** FIXED — commit `c6a67e0` บน `harden/2026-09-05` (ยังไม่ deploy)
...
- **Evidence:**
  ...
  ```
- **Remediation (คงเหลือ):** deploy `c6a67e0` หลัง owner-present sign-in proof; post-deploy curl: raw host `/`, `/courses`, `/api/leads` ต้องเป็น 404 เปล่า, canonical ต้องยังเป็น Access 302; อัปเดต inventory/runbook ที่คาดหวังว่า raw route ตอบ 200; พิจารณา `workers_dev:false` เป็นชั้นที่สอง (commit message อธิบายว่าเลือก code policy เพราะพิสูจน์ด้วย test ได้และไม่หายเมื่อ config ถูกทับ).
- **Sources:** both (Fable #35, GLM high) · **Status:** FIXED (c6a67e0, pending deploy)
...
| DIM-02 | Authorization, IDOR & tenancy | ทุก route/page ใต้ `src/app` ถูก grep: 10 ไฟล์ protected เรียก `currentUser()`+`authorizeCourseResource()`/`getCourseAccess()`; ที่ไม่เรียกคือ public/auth/dashboard shell; `course-access.ts` 4 ชั้น; 0019 grants + `privileged/academy-data-api-roles.sql`; 0018 staff grants | CHECKED | 2026-09-05 | SEC-ACADEMY-016/020; `/player/*` staff guard อ่านผ่าน grep เท่านั้น |
...
| DIM-06 | Infra, deploy & hardening | `worker.ts`, `wrangler.jsonc`, diff `c6a67e0`, assets/`_headers`, `deploy:cf` (`--keep-vars`), grants SQL; Docker/systemd ไม่มีใน tree นี้ (retention API compose ไม่ได้อ่านลึก) | CHECKED | 2026-09-05 | SEC-ACADEMY-013/014/027(deploy) |
...
| SEC-ACADEMY-002 | Auth gate wiring | production branch ใน middleware (syntactic prefilter, 401 JSON สำหรับ `/api/*`), `/api/auth/me` ผ่าน production store, unit test `NODE_ENV=production`, smoke GET `/dashboard` ด้วย cookie จริง | OPEN | 2026-09-05 | ทั้งหมด — ต้องแก้ก่อน owner-present journey ข้อ 1 |
...
| SEC-ACADEMY-027 | Edge exposure | host allowlist ใน `worker.ts` + `edge-host-policy.ts` + unit test | DONE (c6a67e0) | 2026-09-05 | deploy + post-deploy curl proof (raw 404 / canonical 302) + อัปเดต inventory/runbook |

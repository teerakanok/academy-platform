...
| ด่าน | ผล |
|---|---|
| unit ทั้ง repo | 1348 ข้อผ่าน (117 ไฟล์) |
| เลน signer + ด่าน not-wired + sandbox | 73 ข้อผ่าน บนทั้ง node 24.18.0 (engine ที่ประกาศ) และ 25.5.0 |
| `tsc --noEmit` | สะอาด |
...
| `scripts/adversarial/sandbox-exit-path-poc.mjs` | ยิงเส้นทาง exit จริง: restore ล้ม EACCES สองครั้ง โปรเซสออก แล้วยังกู้ไบต์เดิมจากดิสก์ได้ (exit 0) |
| `assertPristine` ก่อนเริ่มทุกสคริปต์ที่แก้ไฟล์จริง | ทำสกปรกเองแล้วยิง — ทั้งสองสคริปต์ปฏิเสธด้วยรหัส `2` และไม่แตะไฟล์ใดๆ |
...
2. **รอบเจ็ด `C0/H0/M1/L1 — REJECT` — in-bound ทั้งสองปิดแล้ว (2026-08-15)**

   รอบเจ็ดยืนยันว่า M-01 (ยึดพอร์ต) ปิดจริงภายใน threat model ที่ประกาศไว้ และ
   ตรวจตัวเลขหลักฐานตรงเกือบทั้งหมด แต่ **REJECT** ด้วยสองข้อ ซึ่ง in-bound ทั้งคู่
   ตามเกณฑ์ §3 ของ `checkpoint-ril` แม้จะเป็นเรื่องของ *เครื่องมือที่ใช้พิสูจน์*
   เพราะเข้าข้อยกเว้นสองข้อพอดี — ทำข้อมูลหายโดยไม่ต้องมีผู้โจมตี และหักล้างข้อความ
   ที่เขียนลงหลักฐานไปแล้ว

   | Finding | In-bound? | เหตุผล |
   |---|---|---|
   | M-02 `restoreOnExit` ปล่อยโปรเซสออกทั้งที่ `pending() > 0` | ใช่ | (ก) ทำข้อมูลหายโดยไม่ต้องมีผู้โจมตี · (ค) หักล้างบรรทัดในรายงานนี้ที่เขียนว่าเคสนี้ปิดแล้ว |
   | L ตัวเลขเลนที่แตะไม่ตรง HEAD (เขียน 66 จริง 68) | ใช่ | (ค) ตัวเลขในหลักฐานที่ตรวจสอบได้แล้วไม่ตรง |

   - **M-02 ปิดแล้ว** — รากคือ snapshot อยู่แต่ในหน่วยความจำ "เก็บไว้ลองซ้ำ" ช่วยได้
     เฉพาะตอนที่ยังมีใครเรียกซ้ำ ถ้า `EACCES` นานเกินสองครั้งที่ handler เรียกติดกัน
     ไบต์เดิมหายไปพร้อมโปรเซส ตอนนี้ **ทุกทางที่ปล่อยไบต์เดิมทิ้ง** (คืนไม่สำเร็จ,
     ไฟล์ถูกสลับ, มีคนแก้ทับ) เขียนสำเนาลงดิสก์นอกรีโปพร้อม `manifest.json`
     (พาธเป้าหมาย + sha256 + จำนวนไบต์) แล้ว exit handler พิมพ์คำสั่ง `cp` ที่กู้ได้จริง
     · ยืนยันด้วย POC เดียวกับที่ reviewer ใช้ REJECT: child exit `0`, ไฟล์ยังเป็น
     `HARNESS-BYTES`, แต่กู้ `ORIGINAL-UNCOMMITTED` กลับได้จากดิสก์หลังโปรเซสตายแล้ว
     · POC commit ไว้ที่ `scripts/adversarial/sandbox-exit-path-poc.mjs` รันซ้ำได้
   - **L ปิดแล้ว** — นับใหม่จากการรันจริง: เลนที่แตะ 72 ข้อ (เพิ่มเทส spill 4 ข้อ)
     บนทั้ง node 24.18.0 และ 25.5.0 · unit ทั้ง repo 1347 ข้อ
   - แถมจากรอบเดียวกัน: `problems()` เคยสะสมข้อความซ้ำทุกครั้งที่ `restore()` ถูกเรียกซ้ำ
     ตอนนี้ข้อความเดียวกันบันทึกครั้งเดียว (ยังไม่ใช่การล้าง error เก่าหลัง retry สำเร็จ
     ซึ่งยังเป็นหนี้ที่ reviewer ระบุไว้)
   - **เจอเพิ่มระหว่างปิด M-02 และปิดไปด้วย: สัญญาณที่จับไม่ได้ทำให้ความเสียหาย
     กลายเป็น baseline** — ตอนรันสคริปต์โจมตี กระบวนการถูก timeout ของเครื่องมือฆ่า
     ด้วย `SIGKILL` ทั้ง process group ซึ่งไม่ผ่าน handler ใดๆ `tests/workerd/wrangler.jsonc`
     จึงค้างอยู่ในสภาพที่ถูกแก้ (BOM + trailing comma) แล้วรอบถัดไป **อ่านสภาพนั้นเป็น
     "ของเดิม"** ผูกความเสียหายเข้าเป็น baseline ถาวรและรายงาน `SURVIVED` ลวงว่าเลนปกติพัง
     ทั้งที่ผลิตภัณฑ์ไม่ได้เปลี่ยนอะไร · ปิดด้วย `assertPristine(root, paths)` ที่ทั้งสอง
     สคริปต์เรียกก่อนแตะไฟล์แรก: ถ้าไฟล์ที่จะแก้ไม่ตรงกับ HEAD จะออกด้วยรหัส `2`
     พร้อมคำสั่ง `git checkout --` ที่คืนได้จริง แทนที่จะเดินต่อ · ไฟล์ที่ค้างคืนแล้วและ
     ยืนยันว่าตรงกับ freeze manifest (`924021045622…`, 670 ไบต์)

3. **รอบหก `C0/H0/M2/L0` — in-bound ทั้งสองปิดแล้ว**
   ...
   - **Sandbox ทิ้ง snapshot หลัง restore ล้มเหลวชั่วคราว** — ของเดิมล้างบัญชีทิ้งทุกกรณี
     ...
     **(รอบเจ็ดหักล้างว่ายังไม่พอ — ครอบเฉพาะการเรียกซ้ำด้วยมือ ไม่ครอบเส้นทาง exit
     ดูข้อ 2 ด้านบนสำหรับการปิดจริง)**
...
4. **`REQUIRED_CHECKS` ต้องแก้ด้วยมือ** — เพิ่ม check ใน worker แล้วลืมอัปเดตรายการ
   จะทำให้เลน fail ตั้งใจให้เป็นแบบนั้น ลืมแล้ว fail ดีกว่าลืมแล้วเงียบ

5. **การคืน `importKey` หลัง patch ใน worker** — ใช้ `Reflect.deleteProperty` ซึ่งคืน
...
6. **ยังไม่ต่อเข้า production** — บังคับด้วยด่าน

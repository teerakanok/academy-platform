...
try {
  ...
  const override = mode === 'candidate' ? { 'Cloudflare-Workers-Version-Overrides': 'cyberskills-academy="8e4d5c04-6b6f-459b-99e3-5b2352d0f133"' } : {};
  ...
  for (const course of ['sign-in']) for (const locale of ['en']) for (const [name, viewport] of [['desktop', { width: 1440, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    try {
      ...
      const response = await page.goto(origin + '/sign-in', { waitUntil: 'networkidle', timeout: 60000 });
      const h1 = await page.locator('h1').innerText();
      const url = page.url();
      const overflow = await page.evaluate(() => document.documentElement.scrollWidth > innerWidth);
      const pass = response?.status() === 200 && url === origin + '/sign-in' && h1.trim().length > 0 && !overflow;
      if (pass) await page.screenshot({ path: `${root}/academy-cookie-${mode}-${course}-${locale}-${name}.png` });
  ...
  writeFileSync(`${root}/academy-cookie-browser-${mode}.json`, JSON.stringify(result, null, 2) + '\n');

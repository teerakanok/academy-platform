...
try{
 ...
 let previous;
 for(let i=0;i<2;i++){
  const res=await fetch(origin+'/courses/basic-os-linux/en',{headers:{'x-nonce':'attacker-probe','content-security-policy':"script-src 'unsafe-inline'"}});const html=await res.text();writeFileSync(out+'/academy-csp-workerd-body-'+i+'-r9.html',html,{flag:'wx'});const csp=res.headers.get('content-security-policy')||'',nonce=csp.match(/'nonce-([^']+)'/)?.[1];
 ...
 for(const [name,viewport] of [['desktop',{width:1440,height:900}],['mobile',{width:390,height:844}]])for(const theme of ['light','dark']){
  const context=await browser.newContext({viewport,colorScheme:theme});await context.route('**/*',r=>new URL(r.request().url()).origin===origin?r.continue():r.abort());const page=await context.newPage();let errors=[],consoleErrors=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error'&&consoleErrors.length<20)consoleErrors.push(m.text())});
  const navigation=await page.goto(origin+'/courses/basic-os-linux/en',{waitUntil:'networkidle'});result.browserHeaders={status:navigation.status(),csp:await navigation.headerValue('content-security-policy'),cache:await navigation.headerValue('cache-control')};writeFileSync(out+'/academy-csp-workerd-browser-body-r9.html',await navigation.text(),{flag:'wx'});await page.getByTestId('public-course-syllabus').waitFor({timeout:10000}).catch(async e=>{await page.screenshot({path:out+'/academy-csp-workerd-failure-r9.png'});result.diagnostic={url:page.url(),title:await page.title(),body:(await page.locator('body').innerText()).slice(0,2000),pageErrors:errors,consoleErrors};throw e;});
  if(theme==='dark'){await page.getByTestId('theme-toggle').click();await page.reload({waitUntil:'networkidle'});await page.getByTestId('public-course-syllabus').waitFor({timeout:10000}).catch(async e=>{await page.screenshot({path:out+'/academy-csp-workerd-failure-r9.png'});result.diagnostic={url:page.url(),title:await page.title(),body:(await page.locator('body').innerText()).slice(0,2000),pageErrors:errors,consoleErrors};throw e;});}if(await page.locator('html').getAttribute('data-theme')!==theme)throw Error('ACTUAL_THEME_MISMATCH');
  ...
  const capture=out+`/academy-csp-workerd-${name}-${theme}-r9.png`;if(new URL(page.url()).origin!==origin)throw Error('CAPTURE_TARGET_CHANGED');await page.screenshot({path:capture});

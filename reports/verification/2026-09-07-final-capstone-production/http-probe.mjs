...
const origin='https://academy.cyberskills.co.th'; const version='f67cb693-8faf-416d-a598-eaf77b9f637d';
...
}catch(error){result.failure='ACCESS_OR_HTTP_PROBE_FAILED';result.error_type=String(error?.name??'unknown');result.cause_code=/^[A-Z0-9_]+$/.test(error?.cause?.code??'')?error.cause.code:'unknown';}
result.pass=!result.failure&&result.checks.length===25&&result.checks.every(x=>x.pass);if(!result.pass)process.exitCode=1;
writeFileSync(`/private/tmp/cyberskills-prod-cde63a58/records/academy-final-capstone-${mode}-http.json`,JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));

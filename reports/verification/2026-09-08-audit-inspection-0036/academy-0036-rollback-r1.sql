...
DO $guard$ BEGIN
IF current_database()<>'postgres' OR current_user<>'supabase_admin' THEN RAISE EXCEPTION 'wrong target or operator'; END IF;
...
IF (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='academy' AND c.relname IN ('staff_role_audit','course_entitlement_audit') AND c.relowner='postgres'::regrole AND c.relrowsecurity AND NOT c.relforcerowsecurity)<>2 THEN RAISE EXCEPTION 'audit table baseline drift'; END IF;
